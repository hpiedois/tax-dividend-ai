...
export function SettingsMenu({ onOpenTaxProfile, onOpenHelp, onLogout }: SettingsMenuProps) {
    ...
    const THEMES: Array<{ code: Theme; labelKey: string; icon: typeof Sun }> = [
        { code: 'light', labelKey: 'settings.theme_light', icon: Sun },
        { code: 'dark', labelKey: 'settings.theme_dark', icon: Moon },
        { code: 'system', labelKey: 'settings.theme_system', icon: Monitor },
    ];
    ...
    return (
        <div className="space-y-6 font-sans">
            ...
            <div className="relative z-30">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 px-1 font-heading">{t('settings.section_appearance')}</h3>
                                ...
                                >
                                    {THEMES.map((themeOption) => (
                                        <button
                                            ...
                                            onClick={() => { setTheme(themeOption.code); setOpenDropdown(null); }}
                                        ...
                                        >
                                            <span className="flex items-center gap-2">
                                                ...
                                                <span className={theme === themeOption.code ? 'font-medium' : ''}>{t(themeOption.labelKey)}</span>
                                            </span>
                                            {theme === themeOption.code && <Check className="w-3 h-3 text-brand-600" />}
                                        </button>
                                    ))}
                                </motion.div>
                            )}
            ...
            <div className="relative z-20">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 px-1 font-heading">{t('settings.section_language')}</h3>
                <Card className="p-0 overflow-visible bg-card/50 backdrop-blur-sm">
                    <div className="relative">
                        <div className="flex items-center justify-between p-4">
            ...
            {/* Account Section */}
            <div>
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 px-1 font-heading">{t('settings.section_account')}</h3>
                <Card className="p-0 overflow-hidden divide-y divide-border bg-card/50 backdrop-blur-sm">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-foreground">{t('settings.tax_profile')}</span>
                        </div>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-foreground">{t('settings.help')}</span>
                        </div>
                        <ChevronRight className="w-4 h-4 text-muted-foreground" />
                    </button>
                </Card>
