...
class StorageServiceTest {
        ...
        void shouldUploadFromInputStream() throws Exception {
                ...
                FileUploadResultDTO result = storageService.uploadFile(
                                inputStream, fileName, "application/pdf", "test-folder");

                // Then
                assertThat(result).isNotNull();
                assertThat(result.getSuccess()).isTrue();
                assertThat(result.getS3Key()).startsWith("test-folder/");
                assertThat(result.getS3Key()).contains("test.pdf");
                assertThat(result.getFileSize()).isEqualTo(content.length);
        ...
        void shouldThrowExceptionWhenGeneratingUrlFails() throws Exception {
                ...
                // When/Then
                assertThatThrownBy(() -> storageService.generatePresignedUrl(s3Key, expiration))
                                .isInstanceOf(StorageException.class)
        ...
        void shouldDeleteMultipleFiles() throws Exception {
                ...
                Iterable<Result<DeleteError>> emptyResults = java.util.Collections.emptyList();
        ...
        void shouldHandlePartialDeletionFailures() throws Exception {
                ...
                // Mocking all success (empty error list) because constructing DeleteError
                // results is complex
                Iterable<Result<DeleteError>> emptyResults = java.util.Collections.emptyList();
                when(minioClient.removeObjects(any(RemoveObjectsArgs.class)))
                                .thenReturn(emptyResults);
                ...
                // Then - 3 out of 3 should succeed (mock returns empty errors)
                assertThat(deletedCount).isEqualTo(3);
                verify(minioClient).removeObjects(any(RemoveObjectsArgs.class));
        }
