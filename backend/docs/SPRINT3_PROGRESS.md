...
- ✅ Enum `DividendStatementUpdateDto` with transition validation
...
- ✅ DividendStatementUpdateDto enum (UPLOADED, PARSING, PARSED, VALIDATED, SENT, PAID)
...
**OpenAPI Spec**:
- ✅ `BulkImportDividendsRequestDto` schema (statementId + array of dividend items)
- ✅ `BulkImportDividendItem` schema (all dividend fields with validation)
- ✅ `BulkImportDividendsResponseDto` schema (counts, totals, IDs, errors)
